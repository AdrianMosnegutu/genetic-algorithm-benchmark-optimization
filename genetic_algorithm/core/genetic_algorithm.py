...
class GeneticAlgorithm:
    ...
    def __init__(self, objective_function: Callable, domain: List[List[float]], 
                 representation: str = 'real', crossover_type: str = 'arithmetic', 
                 population_size: int = 50, mutation_rate: float = None, 
                 crossover_rate: float = 0.8, max_evaluations: int = 10000):
        ...
        self.mutation_rate = mutation_rate if mutation_rate is not None else (0.05 if representation == 'binary' else 0.02)
    ...
    def create_individual(self) -> Individual:
        ...
        if self.representation == 'binary':
            ...
            genes = [random.uniform(self.domain[i][0], self.domain[i][1]) for i in range(2)]
            return Individual(genes)
    ...
    def crossover_binary_1point(self, parent1: Individual, parent2: Individual) -> Tuple[Individual, Individual]:
    ...
    def crossover_binary_2point(self, parent1: Individual, parent2: Individual) -> Tuple[Individual, Individual]:
    ...
    def crossover_arithmetic(self, parent1: Individual, parent2: Individual) -> Tuple[Individual, Individual]:
        ...
        for i in range(len(child1_genes)):
            child1_genes[i] = np.clip(child1_genes[i], self.domain[i][0], self.domain[i][1])
            child2_genes[i] = np.clip(child2_genes[i], self.domain[i][0], self.domain[i][1])
        
        return Individual(child1_genes), Individual(child2_genes)
    
    def crossover_blx_alpha(self, parent1: Individual, parent2: Individual, alpha: float = 0.5) -> Tuple[Individual, Individual]:
        ...
        if self.representation == 'binary':
            ...
            for i in range(len(mutated_genes)):
                if random.random() < self.mutation_rate:
                    ...
                    mutated_genes[i] = np.clip(mutated_genes[i] + mutation, 
                                             self.domain[i][0], self.domain[i][1])
            return Individual(mutated_genes)
    ...
    def run(self) -> Dict[str, Any]:
        ...
        while self.evaluation_count < self.max_evaluations:
            ...
            while len(new_population) < self.population_size and self.evaluation_count < self.max_evaluations:
                ...
                if self.evaluation_count < self.max_evaluations and len(new_population) < self.population_size - 1:
